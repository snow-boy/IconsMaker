...
bool QICNSHandler::write(const QList<QImage> &images)
{
    QIODevice *device = this->device();
    ...
    QVector<ICNSBlockHeader> block_headers;
    QVector<QByteArray> images_data;

    for(QImage image: images){
        if (image.isNull() || qMin(image.width(), image.height()) == 0){
            return false;
        }
        ...
        block_headers.append(iconEntry);
        images_data.append(imageData);
    }

    ICNSBlockHeader fileHeader;
    fileHeader.ostype = ICNSBlockHeader::TypeIcns;
    fileHeader.length = ICNSBlockHeaderSize*2 + ICNSBlockHeaderSize*block_headers.size()*2;
    for(int i = 0 ; i < block_headers.size(); ++i){
        fileHeader.length += images_data[i].size();
    }
    ...
    ICNSBlockHeader tocHeader;
    tocHeader.ostype = ICNSBlockHeader::TypeToc;
    tocHeader.length = ICNSBlockHeaderSize*block_headers.size() + ICNSBlockHeaderSize;
    stream << tocHeader;

    for(int i = 0 ; i < block_headers.size(); ++i){
        stream << block_headers[i];
    }

    for(int i = 0 ; i < block_headers.size(); ++i){
        stream << block_headers[i];
        stream.writeRawData(images_data[i].constData(), images_data[i].size());
    }
    return stream.status() == QDataStream::Ok;
}
